...
router.post("/login", (req, res) => {
  ...
  import_credential_svc.default.verify(username, password).then((user) => generateToken(user)).then((token) => res.status(200).send({ token })).catch(() => res.status(401).send("Unauthorized"));
});
